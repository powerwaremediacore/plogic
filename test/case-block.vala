/* -*- Mode: vala; indent-tabs-mode: t; c-basic-offset: 2; tab-width: 2 -*-  */
/*
 * Copyright (C) 2016 Daniel Espinosa <daniel.espinosa@pwmc.mx>
 */
using Plg;

public class PlogicTest.CaseBlock
{
	public static void add_funcs ()
	{
		Test.add_func ("/plogic/case/block-or/no-evaluation",
		() => {
			var b = new Plg.GBlock ();
			assert (b.name == "Block1");
			var op = new Plg.GOr ();
			assert (op.enable);
			assert (op.name == "OR1");
			assert (b.get_operators ().size == 0);
			b.add_operator (op);
			assert (b.get_operators ().size == 1);
			var i1 = new Plg.GInput ();
			i1.name = "Input1";
			var i2 = new Plg.GInput ();
			i1.name = "Input2";
			op.get_inputs ().set (i1.name, i1);
			op.get_inputs ().set (i2.name, i2);
			assert (op.get_inputs ().size == 2);
			op.evaluate ();
			assert (!op.get_evaluated ());
			assert (i1.state);
			assert (i1.enable);
			assert (i2.state);
			assert (i2.enable);
			assert (op.get_parent () != null);
			assert (op.get_parent ().name == "Block1");
			assert (op.get_output ().state == false);
			i1.state = false;
			assert (i1.state == false);
			op.evaluate ();
			assert (!op.get_evaluated ());
			assert (op.get_output ().state == false);
			i1.state = true;
			i2.state = false;
			op.evaluate ();
			assert (!op.get_evaluated ());
			assert (op.get_output ().state == false);
			i1.state = false;
			i2.state = false;
			op.evaluate ();
			assert (!op.get_evaluated ());
			assert (op.get_output ().state == false);
		});
		Test.add_func ("/plogic/case/block-or/connected",
		() => {
			var b = new Plg.GBlock ();
			assert (b.name == "Block1");
			var op = new Plg.GOr ();
			assert (op.enable);
			assert (op.name == "OR1");
			assert (b.get_operators ().size == 0);
			b.add_operator (op);
			assert (b.get_operators ().size == 1);
			var i1 = new Plg.GInput ();
			i1.name = "Input1";
			var i2 = new Plg.GInput ();
			i1.name = "Input2";
			op.get_inputs ().set (i1.name, i1);
			op.get_inputs ().set (i2.name, i2);
			assert (op.get_inputs ().size == 2);
			var bi1 = new Plg.GInput ();
			bi1.name = "Input1";
			b.get_inputs ().set (bi1.name, bi1);
			assert (bi1.state == true);
			assert (bi1.enable == true);
			var bi2 = new Plg.GInput ();
			bi2.name = "Input2";
			assert (bi2.state == true);
			assert (bi2.enable == true);
			b.get_inputs ().set (bi2.name, bi2);
			assert (b.get_inputs ().size == 2);
			var c1 = new GConnection ();
			c1.value = "Input1"; // Block.Input2
			op.get_inputs ().get ("Input1").connection = c1;
			var c2 = new GConnection ();
			c2.value = "Input2"; // Block.Input2
			op.get_inputs ().get ("Input2").connection = c2;
			var bo1 = new Plg.GOutput ();
			assert (bo1.name == "Output1");
			b.get_outputs ().set (bo1.name, bo1);
			var c3 = new GConnection ();
			c3.value = "Output1";
			op.get_output ().connections.add (c3);
			b.evaluate ();
			assert (op.get_evaluated ());
			assert (b.get_outputs ().get ("Output1").state == true);
			bi1.state = false;
			b.evaluate ();
			assert (op.get_evaluated ());
			assert (b.get_outputs ().get ("Output1").state == true);
			bi2.state = false;
			b.evaluate ();
			assert (op.get_evaluated ());
			assert (op.get_output ().state == false);
			assert (b.get_outputs ().get ("Output1").state == false);
		});
		Test.add_func ("/plogic/case/block-2or/connected",
		() => {
			var b = new Plg.GBlock ();
			assert (b.name == "Block1");
			var op1 = new Plg.GOr ();
			assert (op1.enable);
			assert (op1.name == "OR1");
			assert (b.get_operators ().size == 0);
			b.add_operator (op1);
			assert (b.get_operators ().size == 1);
			var i11 = new Plg.GInput ();
			i11.name = "Input1";
			var i12 = new Plg.GInput ();
			i12.name = "Input2";
			op1.get_inputs ().set (i11.name, i11);
			op1.get_inputs ().set (i12.name, i12);
			assert (op1.get_inputs ().size == 2);
			var bi1 = new Plg.GInput ();
			bi1.name = "Input1";
			b.get_inputs ().set (bi1.name, bi1);
			assert (bi1.state == true);
			assert (bi1.enable == true);
			var bi2 = new Plg.GInput ();
			bi2.name = "Input2";
			assert (bi2.state == true);
			assert (bi2.enable == true);
			b.get_inputs ().set (bi2.name, bi2);
			assert (b.get_inputs ().size == 2);
			var c1 = new GConnection ();
			c1.value = "Input1"; // Block.Input2
			op1.get_inputs ().get ("Input1").connection = c1;
			var c2 = new GConnection ();
			c2.value = "Input2"; // Block.Input2
			op1.get_inputs ().get ("Input2").connection = c2;
			var bo1 = new Plg.GOutput ();
			assert (bo1.name == "Output1");
			b.get_outputs ().set (bo1.name, bo1);
			// add second OR
			var op2 = new GOr ();
			op2.name = "OR2";
			var i21 = new Plg.GInput ();
			i21.name = "Input1";
			var i22 = new Plg.GInput ();
			i22.name = "Input2";
			op2.get_inputs ().set (i21.name, i21);
			op2.get_inputs ().set (i22.name, i22);
			assert (op2.get_inputs ().size == 2);
			op2.get_output ().name = "Output2";
			assert (b.get_operators ().size == 1);
			b.add_operator (op2);
			assert (b.get_operators ().size == 2);
			var c3 = new GConnection ();
			c3.value = "Output1";
			op1.get_output ().connections.add (c3);
			var bi3 = new GInput ();
			bi3.name = "Input3";
			b.get_inputs ().set (bi3.name, bi3);
			assert (b.get_inputs ().size == 3);
			assert (b.get_inputs ().get ("Input3").state == true);
			var c4 = new GConnection ();
			c4.value = "Input3";
			op2.get_inputs ().get ("Input2").connection = c4;
			var c5 = new GConnection ();
			c5.operator = "OR1";
			c5.value = "Output1";
			op2.get_inputs ().get ("Input1").connection = c5;
			var bo2 = new GOutput ();
			bo2.name = "Output2";
			b.get_outputs ().set (bo2.name, bo2);
			var c6 = new GConnection ();
			c6.value = "Output2";
			op2.get_output ().connections.add (c6);
			b.evaluate ();
			assert (op1.get_evaluated ());
			assert (b.get_outputs ().get ("Output1").state == true);
			assert (op2.get_evaluated ());
			assert (b.get_outputs ().get ("Output2").state == true);
			bi1.state = false;
			b.evaluate ();
			assert (op1.get_evaluated ());
			assert (b.get_outputs ().get ("Output1").state == true);
			assert (op2.get_evaluated ());
			assert (b.get_outputs ().get ("Output2").state == true);
			bi2.state = false;
			b.evaluate ();
			assert (op1.get_evaluated ());
			assert (op1.get_output ().state == false);
			assert (b.get_outputs ().get ("Output1").state == false);
			assert (op2.get_evaluated ());
			assert (b.get_outputs ().get ("Output2").state == true);
			bi3.state = false;
			b.evaluate ();
			assert (op1.get_evaluated ());
			assert (op1.get_output ().state == false);
			assert (b.get_outputs ().get ("Output1").state == false);
			assert (op2.get_evaluated ());
			assert (b.get_outputs ().get ("Output2").state == false);
		});
		Test.add_func ("/plogic/case/block-and/no-evaluation",
		() => {
			var b = new Plg.GBlock ();
			assert (b.name == "Block1");
			var op = new Plg.GAnd ();
			assert (op.enable);
			assert (op.name == "AND1");
			assert (b.get_operators ().size == 0);
			b.add_operator (op);
			assert (b.get_operators ().size == 1);
			var i1 = new Plg.GInput ();
			i1.name = "Input1";
			var i2 = new Plg.GInput ();
			i1.name = "Input2";
			op.get_inputs ().set (i1.name, i1);
			op.get_inputs ().set (i2.name, i2);
			assert (op.get_inputs ().size == 2);
			op.evaluate ();
			assert (!op.get_evaluated ());
			assert (i1.state);
			assert (i1.enable);
			assert (i2.state);
			assert (i2.enable);
			assert (op.get_parent () != null);
			assert (op.get_parent ().name == "Block1");
			assert (op.get_output ().state == true);
			i1.state = false;
			assert (i1.state == false);
			op.evaluate ();
			assert (!op.get_evaluated ());
			assert (op.get_output ().state == true);
			i1.state = true;
			i2.state = false;
			op.evaluate ();
			assert (!op.get_evaluated ());
			assert (op.get_output ().state == true);
			i1.state = false;
			i2.state = false;
			op.evaluate ();
			assert (!op.get_evaluated ());
			assert (op.get_output ().state == true);
		});
		Test.add_func ("/plogic/case/block-and/connected",
		() => {
			var b = new Plg.GBlock ();
			assert (b.name == "Block1");
			var op = new Plg.GAnd ();
			assert (op.enable);
			assert (op.name == "AND1");
			assert (b.get_operators ().size == 0);
			b.add_operator (op);
			assert (b.get_operators ().size == 1);
			var i1 = new Plg.GInput ();
			i1.name = "Input1";
			var i2 = new Plg.GInput ();
			i1.name = "Input2";
			op.get_inputs ().set (i1.name, i1);
			op.get_inputs ().set (i2.name, i2);
			assert (op.get_inputs ().size == 2);
			var bi1 = new Plg.GInput ();
			bi1.name = "Input1";
			b.get_inputs ().set (bi1.name, bi1);
			assert (bi1.state == true);
			assert (bi1.enable == true);
			var bi2 = new Plg.GInput ();
			bi2.name = "Input2";
			assert (bi2.state == true);
			assert (bi2.enable == true);
			b.get_inputs ().set (bi2.name, bi2);
			assert (b.get_inputs ().size == 2);
			var c1 = new GConnection ();
			c1.value = "Input1"; // Block.Input2
			op.get_inputs ().get ("Input1").connection = c1;
			var c2 = new GConnection ();
			c2.value = "Input2"; // Block.Input2
			op.get_inputs ().get ("Input2").connection = c2;
			var bo1 = new Plg.GOutput ();
			assert (bo1.name == "Output1");
			b.get_outputs ().set (bo1.name, bo1);
			var c3 = new GConnection ();
			c3.value = "Output1";
			op.get_output ().connections.add (c3);
			b.evaluate ();
			assert (op.get_evaluated ());
			assert (b.get_outputs ().get ("Output1").state == true);
			bi1.state = false;
			b.evaluate ();
			assert (op.get_evaluated ());
			assert (b.get_outputs ().get ("Output1").state == false);
			bi2.state = false;
			b.evaluate ();
			assert (op.get_evaluated ());
			assert (op.get_output ().state == false);
			assert (b.get_outputs ().get ("Output1").state == false);
		});
		Test.add_func ("/plogic/case/block-2and/connected",
		() => {
			var b = new Plg.GBlock ();
			assert (b.name == "Block1");
			var op1 = new Plg.GAnd ();
			assert (op1.enable);
			assert (op1.name == "AND1");
			assert (b.get_operators ().size == 0);
			b.add_operator (op1);
			assert (b.get_operators ().size == 1);
			var i11 = new Plg.GInput ();
			i11.name = "Input1";
			var i12 = new Plg.GInput ();
			i12.name = "Input2";
			op1.get_inputs ().set (i11.name, i11);
			op1.get_inputs ().set (i12.name, i12);
			assert (op1.get_inputs ().size == 2);
			var bi1 = new Plg.GInput ();
			bi1.name = "Input1";
			b.get_inputs ().set (bi1.name, bi1);
			assert (bi1.state == true);
			assert (bi1.enable == true);
			var bi2 = new Plg.GInput ();
			bi2.name = "Input2";
			assert (bi2.state == true);
			assert (bi2.enable == true);
			b.get_inputs ().set (bi2.name, bi2);
			assert (b.get_inputs ().size == 2);
			var c1 = new GConnection ();
			c1.value = "Input1"; // Block.Input2
			op1.get_inputs ().get ("Input1").connection = c1;
			var c2 = new GConnection ();
			c2.value = "Input2"; // Block.Input2
			op1.get_inputs ().get ("Input2").connection = c2;
			var bo1 = new Plg.GOutput ();
			assert (bo1.name == "Output1");
			b.get_outputs ().set (bo1.name, bo1);
			// add second AND
			var op2 = new GAnd ();
			op2.name = "AND2";
			var i21 = new Plg.GInput ();
			i21.name = "Input1";
			var i22 = new Plg.GInput ();
			i22.name = "Input2";
			op2.get_inputs ().set (i21.name, i21);
			op2.get_inputs ().set (i22.name, i22);
			assert (op2.get_inputs ().size == 2);
			op2.get_output ().name = "Output2";
			assert (b.get_operators ().size == 1);
			b.add_operator (op2);
			assert (b.get_operators ().size == 2);
			var c3 = new GConnection ();
			c3.value = "Output1";
			op1.get_output ().connections.add (c3);
			var bi3 = new GInput ();
			bi3.name = "Input3";
			b.get_inputs ().set (bi3.name, bi3);
			assert (b.get_inputs ().size == 3);
			assert (b.get_inputs ().get ("Input3").state == true);
			var c4 = new GConnection ();
			c4.value = "Input3";
			op2.get_inputs ().get ("Input2").connection = c4;
			var c5 = new GConnection ();
			c5.operator = "AND1";
			c5.value = "Output1";
			op2.get_inputs ().get ("Input1").connection = c5;
			var bo2 = new GOutput ();
			bo2.name = "Output2";
			b.get_outputs ().set (bo2.name, bo2);
			var c6 = new GConnection ();
			c6.value = "Output2";
			op2.get_output ().connections.add (c6);
			b.evaluate ();
			assert (op1.get_evaluated ());
			assert (b.get_inputs ().get ("Input1").state);
			assert (b.get_inputs ().get ("Input2").state);
			assert (b.get_inputs ().get ("Input3").state);
			assert (b.get_outputs ().get ("Output1").state == true);
			assert (op2.get_evaluated ());
			assert (b.get_outputs ().get ("Output2").state == true);
			bi1.state = false;
			b.evaluate ();
			assert (op1.get_evaluated ());
			assert (b.get_outputs ().get ("Output1").state == false);
			assert (op2.get_evaluated ());
			assert (b.get_outputs ().get ("Output2").state == false);
			bi1.state = true;
			bi2.state = false;
			b.evaluate ();
			assert (op1.get_evaluated ());
			assert (op1.get_output ().state == false);
			assert (b.get_outputs ().get ("Output1").state == false);
			assert (op2.get_evaluated ());
			assert (op2.get_output ().state == false);
			assert (b.get_outputs ().get ("Output2").state == false);
			bi1.state = true;
			bi2.state = true;
			bi3.state = true;
			b.evaluate ();
			assert (op1.get_evaluated ());
			assert (op1.get_output ().state == true);
			assert (b.get_outputs ().get ("Output1").state == true);
			assert (op2.get_evaluated ());
			assert (op2.get_output ().state == true);
			assert (b.get_outputs ().get ("Output2").state == true);
			bi3.state = false;
			b.evaluate ();
			assert (op1.get_evaluated ());
			assert (op1.get_output ().state == true);
			assert (b.get_outputs ().get ("Output1").state == true);
			assert (op2.get_evaluated ());
			assert (op2.get_output ().state == false);
			assert (b.get_outputs ().get ("Output2").state == false);
		});
		Test.add_func ("/plogic/case/block-2and-1or/connected",
		() => {
			var b = new Plg.GBlock ();
			assert (b.name == "Block1");
			var op1 = new Plg.GAnd ();
			assert (op1.enable);
			assert (op1.name == "AND1");
			assert (b.get_operators ().size == 0);
			b.add_operator (op1);
			assert (b.get_operators ().size == 1);
			var i11 = new Plg.GInput ();
			i11.name = "Input1";
			var i12 = new Plg.GInput ();
			i12.name = "Input2";
			op1.get_inputs ().set (i11.name, i11);
			op1.get_inputs ().set (i12.name, i12);
			assert (op1.get_inputs ().size == 2);
			var bi1 = new Plg.GInput ();
			bi1.name = "Input1";
			b.get_inputs ().set (bi1.name, bi1);
			assert (bi1.state == true);
			assert (bi1.enable == true);
			var bi2 = new Plg.GInput ();
			bi2.name = "Input2";
			assert (bi2.state == true);
			assert (bi2.enable == true);
			b.get_inputs ().set (bi2.name, bi2);
			assert (b.get_inputs ().size == 2);
			var c1 = new GConnection ();
			c1.value = "Input1"; // Block.Input2
			op1.get_inputs ().get ("Input1").connection = c1;
			var c2 = new GConnection ();
			c2.value = "Input2"; // Block.Input2
			op1.get_inputs ().get ("Input2").connection = c2;
			var bo1 = new Plg.GOutput ();
			assert (bo1.name == "Output1");
			b.get_outputs ().set (bo1.name, bo1);
			// add second AND
			var op2 = new GAnd ();
			op2.name = "AND2";
			var i21 = new Plg.GInput ();
			i21.name = "Input1";
			var i22 = new Plg.GInput ();
			i22.name = "Input2";
			op2.get_inputs ().set (i21.name, i21);
			op2.get_inputs ().set (i22.name, i22);
			assert (op2.get_inputs ().size == 2);
			op2.get_output ().name = "Output2";
			assert (b.get_operators ().size == 1);
			b.add_operator (op2);
			assert (b.get_operators ().size == 2);
			var c3 = new GConnection ();
			c3.value = "Output1";
			op1.get_output ().connections.add (c3);
			var bi3 = new GInput ();
			bi3.name = "Input3";
			b.get_inputs ().set (bi3.name, bi3);
			assert (b.get_inputs ().size == 3);
			assert (b.get_inputs ().get ("Input3").state == true);
			var c4 = new GConnection ();
			c4.value = "Input3";
			op2.get_inputs ().get ("Input2").connection = c4;
			var c5 = new GConnection ();
			c5.operator = "AND1";
			c5.value = "Output1";
			op2.get_inputs ().get ("Input1").connection = c5;
			var bo2 = new GOutput ();
			bo2.name = "Output2";
			b.get_outputs ().set (bo2.name, bo2);
			var c6 = new GConnection ();
			c6.value = "Output2";
			op2.get_output ().connections.add (c6);
			b.evaluate ();
			assert (op1.get_evaluated ());
			assert (b.get_inputs ().get ("Input1").state);
			assert (b.get_inputs ().get ("Input2").state);
			assert (b.get_inputs ().get ("Input3").state);
			assert (b.get_outputs ().get ("Output1").state == true);
			assert (op2.get_evaluated ());
			assert (b.get_outputs ().get ("Output2").state == true);
			bi1.state = false;
			b.evaluate ();
			assert (op1.get_evaluated ());
			assert (b.get_outputs ().get ("Output1").state == false);
			assert (op2.get_evaluated ());
			assert (b.get_outputs ().get ("Output2").state == false);
			bi1.state = true;
			bi2.state = false;
			b.evaluate ();
			assert (op1.get_evaluated ());
			assert (op1.get_output ().state == false);
			assert (b.get_outputs ().get ("Output1").state == false);
			assert (op2.get_evaluated ());
			assert (op2.get_output ().state == false);
			assert (b.get_outputs ().get ("Output2").state == false);
			bi1.state = true;
			bi2.state = true;
			bi3.state = true;
			b.evaluate ();
			assert (op1.get_evaluated ());
			assert (op1.get_output ().state == true);
			assert (b.get_outputs ().get ("Output1").state == true);
			assert (op2.get_evaluated ());
			assert (op2.get_output ().state == true);
			assert (b.get_outputs ().get ("Output2").state == true);
			bi3.state = false;
			b.evaluate ();
			assert (op1.get_evaluated ());
			assert (op1.get_output ().state == true);
			assert (b.get_outputs ().get ("Output1").state == true);
			assert (op2.get_evaluated ());
			assert (op2.get_output ().state == false);
			assert (b.get_outputs ().get ("Output2").state == false);
			var bi4 = new GInput ();
			bi4.name = "Input4";
			b.get_inputs ().set (bi4.name, bi4);
			var op3 = new GOr ();
			op3.name = "OR3";
			var i31 = new GInput ();
			var i32 = new GInput ();
			i32.name = "Input2";
			op3.get_inputs ().set (i31.name, i31);
			op3.get_inputs ().set (i32.name, i32);
			b.add_operator (op3);
			var c7 = new GConnection ();
			c7.operator = "AND2";
			c7.value = "Output1";
			op3.get_inputs ().get ("Input1").connection = c7;
			var c8 = new GConnection ();
			c8.operator = "Input4";
			op3.get_inputs ().get ("Input2").connection = c8;
			var bo3 = new GOutput ();
			bo3.name = "Output3";
			b.get_outputs ().set (bo3.name, bo3);
			var c9 = new GConnection ();
			c9.value = "Output3";
			op3.get_output ().connections.add (c9);
			bi1.state = true;
			bi2.state = true;
			bi3.state = true;
			bi4.state = true;
			b.evaluate ();
			assert (op1.get_evaluated ());
			assert (op1.get_output ().state == true);
			assert (b.get_outputs ().get ("Output1").state == true);
			assert (op2.get_evaluated ());
			assert (op2.get_output ().state == true);
			assert (b.get_outputs ().get ("Output2").state == true);
			assert (op3.get_evaluated ());
			assert (op3.get_output ().state == true);
			assert (b.get_outputs ().get ("Output3").state == true);
			assert (b.get_outputs ().size == 3);
		});
	}
}
